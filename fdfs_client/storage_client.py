...
from fdfs_client.connection import tcp_send_data, tcp_recv_response, ConnectionPool
...
from fdfs_client.utils import appromix, get_file_ext_name
...
def tcp_send_file_ex(conn, filename, buffer_size=4096):
    ...
    with open(filename, "rb") as f:
        ...
        while 1:
            try:
                sent = os.sendfile(sock_fd, in_fd, offset, buffer_size)
                if 0 == sent:
                    break
                nbytes += sent
                offset += sent
...
class Storage_client(object):
    ...
    def storage_delete_file(self, tracker_client, store_serv, remote_filename):
        ...
        try:
            ...
            send_buffer = struct.pack(
                del_fmt, store_serv.group_name.encode(), remote_filename.encode()
            )
            tcp_send_data(store_conn, send_buffer)
            th.recv_header(store_conn)
            # if th.status == 2:
        ...
        return "Delete file successes.", remote_filename, store_serv.ip_addr
    ...
    def _storage_do_download_file(
        ...
        try:
            ...
            if download_type == FDFS_DOWNLOAD_TO_FILE:
                ...
                file_buffer, total_recv_size = tcp_recv_response(store_conn, th.pkg_len)
        except Exception:
            raise
        finally:
            self.pool.release(store_conn)
    ...
    def storage_get_metadata(self, tracker_client, store_serv, remote_file_name):
        ...
        try:
            ...
            send_buffer = struct.pack(
                meta_fmt, store_serv.group_name.encode(), remote_file_name.encode()
            )
            tcp_send_data(store_conn, send_buffer)
            th.recv_header(store_conn)
            # if th.status == 2:
    ...
    def _storage_do_append_file(
        ...
        try:
            ...
            send_buffer = struct.pack(
                append_fmt, appended_filename_len, file_size, appended_filename.encode()
            )
            tcp_send_data(store_conn, send_buffer)
            if upload_type == FDFS_UPLOAD_BY_FILENAME:
